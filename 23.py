...
# 定义二分类变量和分类变量列表
binary_vars = ['sepsis', 'gender', 'pre_shock', 'Inhalation_Damage', 'new_onset_shock',
...
sepsis_0 = data[data['mortality'] == 0]
sepsis_1 = data[data['mortality'] == 1]
...
for column in data.columns:
    ...
    if column in binary_vars or column in categorical_vars:
        ...
        # 构造列联表并进行卡方检验来计算p值
        contingency_table = pd.crosstab(data['mortality'], data[column])
...
ans = pd.DataFrame(columns=["Variables", "Group", "non-Mortality", "Mortality", "p-value"])
for i in range(1, len(results_df)):
    ...
    if name in binary_vars:
        ...
        data0 = pd.DataFrame([[name, 0, nonSep_0, Sep_0, results_df.loc[i, 'p_value']]], columns=["Variables", "Group", "non-Mortality", "Mortality", "p-value"])
        data1 = pd.DataFrame([[name, 1, nonSep_1, Sep_1, results_df.loc[i, 'p_value']]], columns=["Variables", "Group", "non-Mortality", "Mortality", "p-value"])
        ...
        for j in set(set1.keys()).union(set(set3.keys())):
            ...
            if j in set3.keys():
                ...
                Sep = '0 (0)'
            data0 = pd.DataFrame([[name, j, nonSep, Sep, results_df.loc[i, 'p_value']]], columns=["Variables", "Group", "non-Mortality", "Mortality", "p-value"])
            ans = pd.concat([ans, data0]).reset_index(drop=True)
    else:
        ...
        data0 = pd.DataFrame([[name, "", nonSep, Sep, results_df.loc[i, 'p_value']]], columns=["Variables", "Group", "non-Mortality", "Mortality", "p-value"])
        ans = pd.concat([ans, data0]).reset_index(drop=True)
ans.to_csv('234.csv')
...
significant_df_filtered = significant_df[significant_df['Variable'] != 'K']

# Display the filtered dataframe to the user
print(significant_df_filtered)
significant_df_filtered.to_csv('table2_mortality.csv')
...
models = {
    ...
    "XGBoost":  XGBClassifier(random_state=41, booster="gbtree", max_depth=7),
...
shap_summary_rf_dot_pdf = "./result/roc_total_mortality.pdf"
...
evaluation_metrics_chart_pdf = "./result/MortalityEvaluation_Metrics_Comparison_Chart.pdf"
...
# svm Model
svm_model = XGBClassifier(random_state=41, booster="gbtree", max_depth=7)
...
# X_test_final["re"] = 'tag'
# X_test_final = X_test_final.reset_index(drop=False)
# for i in range(0, len(svm_pred)):
#     X_test_final.loc[i, 're'] = svm_pred[i]
...
Feature_Importance_svm_pdf = "./result/Feature_Importance_XGB_23.pdf"
plt.figure(figsize=(10, 8))
...
saved_model = XGBClassifier(random_state=41, booster="gbtree", max_depth=7)
X = X[significant_df_filtered['Variable'].tolist()]
y = data['mortality']
smote = SMOTE(random_state=42)
X, y = smote.fit_resample(X, y)
# scaler = StandardScaler()
# X = scaler.fit_transfor()
saved_model.fit(X,y)
joblib.dump(saved_model, 'xgb_model.joblib')

# # Initialize the SHAP explainer for the Random Forest model
# rf_model = XGBClassifier(random_state=42, booster="gbtree")
...
# plt.title('SHAP Summary Plot (Dot) for GBT')
# shap_summary_rf_dot_pdf = "./result/SHAP_Summary_XGB_23.pdf"
# plt.savefig(shap_summary_rf_dot_pdf, bbox_inches='tight')
# plt.close()

rf_model = XGBClassifier(random_state=41, booster="gbtree", max_depth=7)
rf_model.fit(X_train_final, y_train_final)
explainer_rf = shap.TreeExplainer(rf_model)
shap_values_rf = explainer_rf.shap_values(X_test_final)
print(shap_values_rf)
shap.summary_plot(shap_values_rf, X_test_final, show=False)
plt.title('SHAP Summary Plot (Dot) for XGB')
shap_summary_rf_dot_pdf = "./result/SHAP_Summary_XGB_23.pdf"
plt.savefig(shap_summary_rf_dot_pdf, bbox_inches='tight')
plt.close()

explainer_rf = shap.Explainer(rf_model, X_train_final)
...
plt.figure()
shap.initjs()
html = shap.force_plot(shap_values_rf[1], matplotlib=False)

# 显示图形
shap.save_html('./result/SHAP_force_plot_sample_1_mortality.html', html)
...
# shap_summary_svm_pdf = "./result/SHAP_Beeswarm_SVM_23.pdf"
# plt.savefig(shap_summary_svm_pdf, bbox_inches='tight')
# plt.close()

# # Plotting SHAP beeswarm plot for the Random Forest model
# shap.summary_plot(shap_values_rf, X_test_final, plot_type="bar", show=False)
# plt.title('SHAP Summary Plot (Bar) for XGBoost')
# shap_summary_rf_bar_pdf = "./result/SHAP_Summary_svm_23_Bar.pdf"
# plt.savefig(shap_summary_rf_bar_pdf, bbox_inches='tight')
# plt.close()
#
# shap.summary_plot(shap_values_rf, X_test_final, show=False)
# plt.title('SHAP Summary Plot (Dot) for XGBoost')
# shap_summary_rf_dot_pdf = "./result/SHAP_Summary_svm_23.pdf"
# plt.savefig(shap_summary_rf_dot_pdf, bbox_inches='tight')
# plt.close()
#
# shap_value_rf = explainer_rf(X_test_final)
# print(shap_value_rf)
# plt.figure()
# shap.plots.beeswarm(shap_value_rf, show=False)
# plt.title('SHAP Beeswarm Plot for XGBoost')
# shap_beeswarm_rf_pdf = "./result/SHAP_Beeswarm_svm_23.pdf"
# plt.savefig(shap_beeswarm_rf_pdf, bbox_inches='tight')
# plt.close()

y_probs_model_x = rf_model.predict_proba(X_test_final)[:, 1]
...
plt.plot(fpr_x, tpr_x, color='blue', lw=2, label=f'XGB AUC = {roc_auc_x:.3f}')
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC Curves: XGB vs SOFA Logistic Regression')
plt.legend(loc='lower right')
shap_summary_rf_dot_pdf = "./result/roc_vs_sofa_mortality.pdf"
plt.savefig(shap_summary_rf_dot_pdf, bbox_inches='tight')
plt.show()